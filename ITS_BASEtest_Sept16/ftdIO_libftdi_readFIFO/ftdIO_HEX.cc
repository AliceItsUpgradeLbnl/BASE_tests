#ifndef lint
static char vcid[] = "$Id: ftdIO_HEX.cc 990 2016-04-08 22:39:25Z gcontin $";
...
extern FT_STATUS writeReg(FT_HANDLE, unsigned short, unsigned short);
extern FT_STATUS readReg(FT_HANDLE, unsigned short, unsigned int &);
...
int main(int argc, char *argv[])
{
  ...
  if (bFill) { // ....Fill Memory....
    ...
    ftStatus = readReg(ftdi, address, dataW);
    cout << "readReg returned " << ftStatus
	 << ", data = " << hex << showbase << dataW << endl;
  }
  else { // ....WRITING.....
    cout << "Writing " << hex << showbase << data
	 << " to register at address " << address << endl;
    ftStatus = writeReg(ftdi, address, data);
    if (ftStatus != FT_OK)
