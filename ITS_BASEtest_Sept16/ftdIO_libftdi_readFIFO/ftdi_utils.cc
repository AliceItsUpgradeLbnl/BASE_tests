#ifndef lint
static char vcid[] = "$Id: ftdi_utils.cc 990 2016-04-08 22:39:25Z gcontin $";
...
FT_STATUS writeReg(FT_HANDLE fthandle, unsigned short address, unsigned short data)
{
  int ftStatus;
  ...
  if (ftStatus == FT_OK) {
    // FT_Write OK
    if (BytesWritten != 8)
      cerr << "FT_Write wrote " << BytesWritten << " expected 8" << endl;
    }
  else {
    // FT_Write Failed
...
FT_STATUS readReg(FT_HANDLE fthandle, unsigned short address, unsigned int &data)
{
  int ftStatus;
